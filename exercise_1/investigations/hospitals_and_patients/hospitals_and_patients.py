...
adjust = lambda score, max, min: (score - min)/(max - min)
...
result = scores.join(surveys, scores.provider_id == surveys.provider_id).select(scores.provider_id, scores.mean_scaled_score, surveys.scaled_survey_score)
