...
ranges = {range['metric_id']: range for range in map(lambda row: row.asDict(), context.sql(rangesSql).collect())}
...
intermediateSql = 'select * from scores where score is not null and metric_id in ({0})'.format(','.join(['\'' + member + '\'' for member in weights.keys()]))
context.sql(intermediateSql).map(lambda row: (row.provider_id, adjust(row.score, row.metric_id))).toDF().selectExpr("_1 as provider_id", "_2 as scaled_score").registerTempTable('intermediate_table')

# Utilize the temporary table to create the average and variance of each scaled score
resultSql = 'select hospital_name, state, mean_scaled_score, variance_scaled_score, sum_scaled_score from (select provider_id, avg(scaled_score) mean_scaled_score, sum(scaled_score) sum_scaled_score, var_samp(scaled_score) variance_scaled_score, count(*) score_count from intermediate_table group by provider_id) a join providers on a.provider_id = providers.provider_id where score_count > {0} order by mean_scaled_score desc'.format(.5 * len(weights))
